...
- Added plugin default limiters. ([#2141](https://github.com/turbot/steampipe-plugin-aws/pull/2141))

_Enhancements_

- Updated the `aws_ebs_snapshot` table to not set the `MaxResults` parameter to `1000` by default in the `listAwsEBSSnapshots` function, as it turns out that AWS will return all snapshots in a single page if we don't set the `MaxResults` parameter. ([#2088](https://github.com/turbot/steampipe-plugin-aws/pull/2088))
