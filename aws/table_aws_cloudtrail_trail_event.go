...
func listCloudTrailEvents(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	...
	plugin.Logger(ctx).Error("listCloudTrailEvents", "lookupAttributes", lookupAttributes)
	...
	if lookupAttributes != nil {
		...
		for k, v := range lookupAttributes {
			plugin.Logger(ctx).Warn("lookupAttributes: ", k, v)
		}
	}
	...
	plugin.Logger(ctx).Error("listCloudTrailEvents", "StartTime", input.StartTime)
	plugin.Logger(ctx).Error("listCloudTrailEvents", "EndTime", input.EndTime)
	...
	err = svc.LookupEventsPages(
		...
		func(page *cloudtrail.LookupEventsOutput, _ bool) bool {
			for count, trailEvent := range page.Events {
				plugin.Logger(ctx).Error("listCloudTrailEvents", "Event No: ", count)
				plugin.Logger(ctx).Error("listCloudTrailEvents", "Event Time: ", *trailEvent.EventTime)
