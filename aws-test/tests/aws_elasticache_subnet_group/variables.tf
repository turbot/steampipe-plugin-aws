...
data "template_file" "resource_aka" {
  template = "arn:$${partition}:elasticache:$${region}:$${account_id}:subnetgroup:${aws_elasticache_subnet_group.named_test_resource.name}"
  vars = {
...
output "resource_aka" {
  depends_on = [aws_elasticache_subnet_group.named_test_resource]
  value      = data.template_file.resource_aka.rendered
}

output "resource_id" {
  value = aws_elasticache_subnet_group.named_test_resource.name
}
