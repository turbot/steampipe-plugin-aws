...
resource "null_resource" "named_test_resource" {
  provisioner "local-exec" {
    command = "aws wellarchitected create-workload --workload-name ${var.resource_name} --description 'testinh-cli'  --environment 'PREPRODUCTION' --review-owner 'khushboo' --lenses 'wellarchitected' --aws-regions 'us-east-1' > ${local.path}"
  }
}
