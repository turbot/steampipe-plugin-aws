...
data "local_file" "execution_history" {
  ...
  filename   = local.execution_history_path
}

output "execution_arn" {
  depends_on = [null_resource.execution]
  value      = jsondecode(data.local_file.execution.content).executionArn
}

output "type" {
  depends_on = [null_resource.execution_history]
  value      = jsondecode(data.local_file.execution_history.content).events[0].type
}
