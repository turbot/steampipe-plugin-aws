...
resource "null_resource" "named_test_resource" {
  provisioner "local-exec" {
    command = <<EOT
      aws redshift create-cluster-snapshot --cluster-identifier ${aws_redshift_cluster.cluster.cluster_identifier} --snapshot-identifier ${var.resource_name} --profile ${var.aws_profile} --region ${var.aws_region};
    EOT
  }
}
