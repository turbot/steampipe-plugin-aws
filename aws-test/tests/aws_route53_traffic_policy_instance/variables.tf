...
variable "ttl" {
  type        = number
  default     = 60
  description = "TTL of the resource used throughout the test."
}

variable "version_input" {
  type        = number
  default     = 1
  description = "Version of the resource used throughout the test."
}
...
output "resource_aka" {
  value = "arn:aws:route53::${data.aws_caller_identity.current.account_id}:trafficpolicyinstance/${aws_route53_traffic_policy_instance.named_test_resource.id}"
}

output "id" {
  value = aws_route53_traffic_policy_instance.named_test_resource.id
}
