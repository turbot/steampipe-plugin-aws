...
data "template_file" "resource_aka" {
  template = "arn:$${partition}:ec2:$${region}:$${account_id}:eip/${aws_eip.named_test_resource.id}"
  vars = {
...
output "resource_id" {
  value = aws_eip.named_test_resource.id
}
...
output "resource_aka" {
  depends_on = [aws_eip.named_test_resource]
